 ...
 * number is only increased when necessary and does not have to be the same size as the number it
 ...
 *
 * Note that SignedMutableBigIntegers only support signed addition and subtraction. All other
 * operations occur as with MutableBigIntegers.
 *
 * @see BigInteger
...
class SignedMutableBigInteger extends MutableBigInteger {
     ...
     */
    SignedMutableBigInteger(final int val) {
        super(val);
    }
     ...
     */
    SignedMutableBigInteger(final MutableBigInteger val) {
        super(val);
    }
     ...
     */
    void signedAdd(final SignedMutableBigInteger addend) {
        if (sign == addend.sign)
            add(addend);
        else
     ...
     * Signed addition built upon unsigned add and subtract.
     */
    void signedAdd(final MutableBigInteger addend) {
        if (sign == 1)
            add(addend);
        else
            sign = sign * subtract(addend);
     ...
     */
    void signedSubtract(final SignedMutableBigInteger addend) {
        if (sign == addend.sign)
            sign = sign * subtract(addend);
        else
            add(addend);
     ...
     * Signed subtraction built upon unsigned add and subtract.
     */
    void signedSubtract(final MutableBigInteger addend) {
        if (sign == 1)
            sign = sign * subtract(addend);
        else
            add(addend);
