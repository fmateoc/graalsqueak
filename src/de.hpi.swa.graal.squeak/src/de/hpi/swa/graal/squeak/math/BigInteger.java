 ...
 * defined in <i>The Java Language Specification</i>. For example, division by zero throws an
 * {@code ArithmeticException}, and division of a negative by a positive yields a negative (or zero)
 * remainder. All of the details in the Spec concerning overflow are ignored, as BigIntegers are
 ...
 * The unsigned right shift operator ({@code >>>}) is omitted, as this operation makes little sense
 * in combination with the "infinite word size" abstraction provided by this class.
 *
 * <p>
 * Semantics of bitwise logical operations exactly mimic those of Java's bitwise integer operators.
 * The binary operators ({@code and}, {@code or}, {@code xor}) implicitly perform sign extension on
 ...
 *
 * <p>
 * Modular arithmetic operations are provided to compute residues, perform exponentiation, and
 * compute multiplicative inverses. These methods always return a non-negative result, between
 * {@code 0} and {@code (modulus - 1)}, inclusive.
 *
 * <p>
 ...
 * operated on, as they affect only a single bit, and the "infinite word size" abstraction provided
 * by this class ensures that there are infinitely many "virtual sign bits" preceding each
 * BigInteger.
 *
 * <p>
 * For the sake of brevity and clarity, pseudo-code is used throughout the descriptions of
 * BigInteger methods. The pseudo-code expression {@code (i + j)} is shorthand for "a BigInteger
 * whose value is that of the BigInteger {@code i} plus that of the BigInteger {@code j}." The
 * pseudo-code expression {@code (i == j)} is shorthand for "{@code true} if and only if the
 * BigInteger {@code i} represents the same value as the BigInteger {@code j}." Other pseudo-code
 * expressions are interpreted similarly.
 *
 * <p>
 * All methods and constructors in this class throw {@code NullPointerException} when passed a null
 ...
 * range of {@code BigInteger}. The range must be at least 1 to 2<sup>500000000</sup>.
 *
 * @implNote BigInteger constructors and operations throw {@code ArithmeticException} when the
 *           result is out of the supported range of -2<sup>{@code Integer.MAX_VALUE}</sup>
...
public class BigInteger extends Number implements Comparable<BigInteger> {
     ...
     */
    @Deprecated private int bitCount;
     ...
     *             is applied in accessor method.
     */
    @Deprecated private int bitLength;
     ...
     *             is applied in accessor method.
     */
    @Deprecated private int lowestSetBit;
     ...
     *             is applied in accessor method.
     */
    @Deprecated private int firstNonzeroIntNum;
     ...
     */
    public static final int MAX_MAG_LENGTH = Integer.MAX_VALUE / Integer.SIZE + 1; // (1 << 26)
     ...
     */
    public static final int KARATSUBA_THRESHOLD = 80;
     ...
     */
    public static final int TOOM_COOK_THRESHOLD = 240;
    ...
    /**
     * The threshold value for using squaring code to perform multiplication of a {@code BigInteger}
     * instance by itself. If the number of ints in the number are larger than this value,
     * {@code multiply(this)} will return {@code square()}.
     */
    public static final int MULTIPLY_SQUARE_THRESHOLD = 20;

    /**
     * The threshold for using an intrinsic version of implMontgomeryXXX to perform Montgomery
     * multiplication. If the number of ints in the number is more than this value we do not use the
     * intrinsic.
     */
     ...
     * into a BigInteger. The input array is assumed to be in <i>big-endian</i> byte-order: the most
    ...
    /**
     * This private constructor translates an int array containing the two's-complement binary
     * representation of a BigInteger into a BigInteger. The input array is assumed to be in
     * <i>big-endian</i> int-order: the most significant int is in the zeroth element.
     */
    private BigInteger(final int[] val) {
        if (val.length == 0) {
            throw new NumberFormatException("Zero length BigInteger");
        }
     ...
     * represented as an integer signum value: -1 for negative, 0 for zero, or 1 for positive. The
     * magnitude is a byte array in <i>big-endian</i> byte-order: the most significant byte is in
     * the zeroth element. A zero-length magnitude array is permissible, and will result in a
     ...
     *             1), or {@code signum} is 0 and {@code magnitude} contains one or more non-zero
     *             bytes.
     */
     ...
     * provided by {@code
     * Character.digit}. The String may not contain any extraneous characters (whitespace, for
     * example).
     *
     ...
     */
    BigInteger(final char[] val, final int sign, final int len) {
     ...
     */
    private BigInteger(final long ival) {
        long val = ival;
        if (val < 0) {
            val = -val;
            signum = -1;
    ...
    private static int parseInt(final char[] source, final int istart, final int end) {
        int start = istart;
    ...
    private static void destructiveMulAdd(final int[] x, final int y, final int z) {
     ...
     * (2<sup>{@code numBits}</sup> - 1), inclusive. The uniformity of the distribution assumes that
     * a fair source of random bits is provided in {@code rnd}. Note that this constructor always
    ...
    private static byte[] randomBits(final int numBits, final Random rnd) {
        if (numBits < 0) {
    ...
    /**
     * Constructs a randomly generated positive BigInteger that is probably prime, with the
     * specified bitLength.
     *
     * <p>
     * It is recommended that the {@link #probablePrime probablePrime} method be used in preference
     * to this constructor unless there is a compelling need to specify a certainty.
     *
     * @param bitLength bitLength of the returned BigInteger.
     * @param certainty a measure of the uncertainty that the caller is willing to tolerate. The
     *            probability that the new BigInteger represents a prime number will exceed (1 -
     *            1/2<sup>{@code certainty}</sup>). The execution time of this constructor is
     ...
     * @see #bitLength()
     */
    public BigInteger(final int bitLength, final int certainty, final Random rnd) {
    ...
    /**
     * Find a random number of the specified bitLength that is probably prime. This method is used
     * for smaller primes, its performance degrades on larger bitlengths.
     *
     * This method assumes bitLength > 1.
     */
    private static BigInteger smallPrime(final int bitLength, final int certainty, final Random rnd) {
    ...
    /**
     * Find a random number of the specified bitLength that is probably prime. This method is more
     * appropriate for larger bitlengths since it uses a sieve to eliminate most composites before
     * using a more expensive test.
     */
    private static BigInteger largePrime(final int bitLength, final int certainty, final Random rnd) {
        BigInteger p;
    ...
    /**
     * Returns the first integer greater than this {@code BigInteger} that is probably prime. The
     * probability that the number returned by this method is composite does not exceed
     * 2<sup>-100</sup>. This method will never skip over a prime when searching: if it returns
     * {@code p}, there is no prime {@code q} such that {@code this < q < p}.
     *
    ...
    private static int getPrimeSearchLen(final int bitLength) {
     ...
     *         composite.
     */
    boolean primeToCertainty(final int certainty, final Random random) {
        int rounds = 0;
     ...
     */
    private static int jacobiSymbol(final int ip, final BigInteger n) {
        int p = ip;
        if (p == 0) {
            return 0;
        }
    ...
    private static BigInteger lucasLehmerSequence(final int z, final BigInteger k, final BigInteger n) {
     ...
     * is taken from the DSA spec (NIST FIPS 186-2).
     *
     * The following assumptions are made: This BigInteger is a positive, odd number greater than 2.
     * iterations<=50.
     */
    private boolean passesMillerRabin(final int iterations, final Random irnd) {
    ...
    static {
        assert 0 < KARATSUBA_THRESHOLD && KARATSUBA_THRESHOLD < TOOM_COOK_THRESHOLD && TOOM_COOK_THRESHOLD < Integer.MAX_VALUE && 0 < KARATSUBA_SQUARE_THRESHOLD &&
                        KARATSUBA_SQUARE_THRESHOLD < TOOM_COOK_SQUARE_THRESHOLD && TOOM_COOK_SQUARE_THRESHOLD < Integer.MAX_VALUE : "Algorithm thresholds are inconsistent";
    ...
    /**
     * Package private methods used by BigDecimal code to add a BigInteger with a long. Assumes val
     * is not equal to INFLATED.
     */
    BigInteger add(final long val) {
        if (val == 0) {
            return this;
        }
        if (signum == 0) {
    ...
    /**
     * Adds the contents of the int array x and long value val. This method allocates a new int
     * array to hold the answer and returns a reference to that array. Assumes x.length &gt; 0 and
     * val is non-negative
     */
    private static int[] add(final int[] x, final long val) {
        final int[] y;
        long sum = 0;
        int xIndex = x.length;
        final int[] result;
     ...
     */
    private static int[] add(final int[] ix, final int[] iy) {
    ...
    private static int[] subtract(final long val, final int[] little) {
        final int highWord = (int) (val >>> 32);
        if (highWord == 0) {
    ...
    /**
     * Subtracts the contents of the second argument (val) from the first (big). The first int array
     * (big) must represent a larger number than the second. This method allocates the space
     * necessary to hold the answer. assumes val &gt;= 0
     */
    private static int[] subtract(final int[] big, final long val) {
        final int highWord = (int) (val >>> 32);
     ...
     */
    private static int[] subtract(final int[] big, final int[] little) {
        int bigIndex = big.length;
        final int[] result = new int[bigIndex];
    ...
    /**
     * Returns a BigInteger whose value is {@code (this * val)}. If the invocation is recursive
     * certain overflow checks are skipped.
     *
     * @param val value to be multiplied by this BigInteger.
     * @param isRecursion whether this is a recursive invocation
     * @return {@code this * val}
     */
    private BigInteger multiply(final BigInteger val, final boolean isRecursion) {
            ...
            if (xlen < TOOM_COOK_THRESHOLD && ylen < TOOM_COOK_THRESHOLD) {
                ...
                if (!isRecursion) {
                    ...
                    if (bitLength(mag, mag.length) +
                                    bitLength(val.mag, val.mag.length) > 32L * MAX_MAG_LENGTH) {
                        reportOverflow();
                    }
                }
    ...
    public static BigInteger multiplyByInt(final int[] x, final int y, final int sign) {
     ...
     */
    public BigInteger multiply(final long iv) {
        long v = iv;
        if (v == 0 || signum == 0) {
            return ZERO;
        }
     ...
     */
    public static int[] multiplyToLen(final int[] x, final int xlen, final int[] y, final int ylen, final int[] iz) {
     ...
     */
    public static BigInteger multiplyKaratsuba(final BigInteger x, final BigInteger y) {
     ...
     *
     */
    public static BigInteger multiplyToomCook3(final BigInteger a, final BigInteger b) {
        ...
        final BigInteger a0, a1, a2, b0, b1, b2;
        ...
        final BigInteger v0, v1, v2, vm1, vinf;
        BigInteger t1;
        BigInteger t2;
        BigInteger tm1;
        BigInteger da1;
     ...
     */
    private BigInteger getToomSlice(final int lowerSize, final int upperSize, final int slice,
                    final int fullsize) {
        int start;
        final int end, sliceSize, len, offset;
    ...
    /**
     * Returns a BigInteger whose value is {@code (this<sup>2</sup>)}. If the invocation is
     * recursive certain overflow checks are skipped.
     *
     * @param isRecursion whether this is a recursive invocation
     * @return {@code this<sup>2</sup>}
     */
     ...
     */
    private static int[] squareToLen(final int[] x, final int len, final int[] iz) {
     ...
     */
    private static void implSquareToLenChecks(final int[] x, final int len, final int[] z, final int zlen) throws RuntimeException {
        if (len < 1) {
     ...
     */
    private static int[] implSquareToLen(final int[] x, final int len, final int[] z, final int zlen) {
        /*
         * The algorithm used here is adapted from Colin Plumb's C library. Technique: Consider the
         * partial products in the multiplication of "abcde" by itself:
         *
         * a b c d e * a b c d e ================== ae be ce de ee ad bd cd dd de ac bc cc cd ce ab
         * bb bc bd be aa ab ac ad ae
         *
         * Note that everything above the main diagonal: ae be ce de = (abcd) * e ad bd cd = (abc) *
         * d ac bc = (ab) * c ab = (a) * b
         *
         ...
         *
         * This is accumulated beginning with the diagonal (which consist of the squares of the
         * digits of the input), which is then divided by two, the off-diagonal added, and
         * multiplied by two again. The low bit is simply a copy of the low bit of the input, so it
    ...
    private BigInteger divideKnuth(final BigInteger val) {
        ...
        return q.toBigInteger(signum * val.signum);
    }
    ...
    private BigInteger[] divideAndRemainderKnuth(final BigInteger val) {
        ...
        result[0] = q.toBigInteger(signum == val.signum ? 1 : -1);
    ...
    private BigInteger remainderKnuth(final BigInteger val) {
        ...
        return a.divideKnuth(b, q).toBigInteger(signum);
    }
    ...
    /**
     * Returns a BigInteger whose value is the greatest common divisor of {@code abs(this)} and
     * {@code abs(val)}. Returns 0 if {@code this == 0 && val == 0}.
     *
     ...
     */
    private static int[] leftShift(final int[] a, final int len, final int n) {
        final int nInts = n >>> 5;
    ...
    static void primitiveRightShift(final int[] a, final int len, final int n) {
        final int n2 = 32 - n;
    ...
    static void primitiveLeftShift(final int[] a, final int len, final int n) {
        if (len == 0 || n == 0) {
            return;
        }
     ...
     * leading zero ints.
     */
    public static int bitLength(final int[] val, final int len) {
        if (len == 0) {
            return 0;
        }
    ...
    public BigInteger mod(final BigInteger m) {
        ...
        final BigInteger result = remainder(m);
     ...
     * @see #modInverse
     */
    public BigInteger modPow(final BigInteger iexponent, final BigInteger m) {
        BigInteger exponent = iexponent;
        if (m.signum <= 0) {
            throw new ArithmeticException("BigInteger: modulus not positive");
        }
        ...
        if (equals(ZERO) && exponent.signum >= 0) {
            return ZERO;
        }
        ...
        final boolean invertResult = exponent.signum < 0;
        if (invertResult) {
            exponent = exponent.negate();
        }

        final BigInteger base = signum < 0 || compareTo(m) >= 0
                        ? mod(m)
                        : this;
        ...
        if (m.testBit(0)) { // odd modulus
            ...
            final BigInteger base2 = signum < 0 || compareTo(m1) >= 0
                            ? mod(m1)
                            : this;

            // Caculate (base ** exponent) mod m1.
            final BigInteger a1 = m1.equals(ONE) ? ZERO : base2.oddModPow(exponent, m1);
    ...
    private static int[] montgomeryMultiply(final int[] a, final int[] b, final int[] n, final int len, final long inv,
                    final int[] iproduct) {
        int[] product = iproduct;
    ...
    private static int[] montgomerySquare(final int[] a, final int[] n, final int len, final long inv,
                    final int[] iproduct) {
        int[] product = iproduct;
    ...
    private static void implMontgomeryMultiplyChecks(final int[] a, final int[] b, final int[] n, final int len, final int[] product) throws RuntimeException {
        if (len % 2 != 0) {
    ...
    // sufficiently large.
    private static int[] materialize(final int[] iz, final int len) {
        int[] z = iz;
    ...
    // intrinsics.
    private static int[] implMontgomeryMultiply(final int[] a, final int[] b, final int[] n, final int len,
                    final long inv, final int[] iproduct) {
        int[] product = iproduct;
        product = multiplyToLen(a, len, b, len, product);
        return montReduce(product, n, len, (int) inv);
    }

    private static int[] implMontgomerySquare(final int[] a, final int[] n, final int len,
                    final long inv, final int[] iproduct) {
        int[] product = iproduct;
        product = squareToLen(a, len, product);
        return montReduce(product, n, len, (int) inv);
     ...
     */
    private BigInteger oddModPow(final BigInteger y, final BigInteger z) {
        /*
         * The algorithm is adapted from Colin Plumb's C library.
         *
         * The window algorithm: The idea is to keep a running product of b1 = n^(high-order bits of
         * exp) and then keep appending exponent bits to it. The following patterns apply to a 3-bit
         * window (k = 3): To append 0: square To append 1: square, multiply by n^1 To append 10:
         * square, multiply by n^1, square To append 11: square, square, multiply by n^3 To append
         * 100: square, multiply by n^1, square, square To append 101: square, square, square,
         * multiply by n^5 To append 110: square, square, multiply by n^3, square To append 111:
         * square, square, square, multiply by n^7
         *
         * Since each pattern involves only one multiply, the longer the pattern the better, except
         * that a 0 (no multiplies) can be appended directly. We precompute a table of odd powers of
         * n, up to 2^k, and can then multiply k bits of exponent at a time. Actually, assuming
         * random exponents, there is on average one zero bit between needs to multiply (1/2 of the
         * time there's none, 1/4 of the time there's 1, 1/8 of the time, there's 2, 1/32 of the
         * time, there's 3, etc.), so you have to do one multiply per k+1 bits of exponent.
         *
         * The loop walks down the exponent, squaring the result buffer as it goes. There is a
         * wbits+1 bit lookahead buffer, buf, that is filled with the upcoming exponent bits. (What
         * is read after the end of the exponent is unimportant, but it is filled with zero here.)
         * When the most-significant bit of this buffer becomes set, i.e. (buf & tblmask) != 0, we
         * have to decide what pattern to multiply by, and when to do it. We decide, remember to do
         * it in future after a suitable number of squarings have passed (e.g. a pattern of "100" in
         * the buffer requires that we multiply by n^1 immediately; a pattern of "110" calls for
         * multiplying by n^3 after one more squaring), clear the buffer, and continue.
         *
         * When we start, there is one more optimization: the result buffer is implcitly one, so
         * squaring it or multiplying by it can be optimized away. Further, if we start with a
         * pattern like "100" in the lookahead window, rather than placing n into the buffer and
         * then starting to square it, we have already computed n^2 to compute the odd-powers table,
         * so we can place that into the buffer and save a squaring.
         *
         * This means that if you have a k-bit window, to compute n^z, where z is the high k bits of
         * the exponent, 1/2 of the time it requires no squarings. 1/4 of the time, it requires 1
         * squaring, ... 1/2^(k-1) of the time, it reqires k-2 squarings. And the remaining
         * 1/2^(k-1) of the time, the top k bits are a 1 followed by k-1 0 bits, so it again only
         * requires k-2 squarings, not k-1. The average of these is 1. Add that to the one squaring
         * we have to do to compute the table, and you'll see that a k-bit window saves k-2
         * squarings as well as reducing the multiplies. (It actually doesn't hurt in the case k =
         * 1, either.)
         */
     ...
     */
    private static int[] montReduce(final int[] n, final int[] mod, final int mlen, final int inv) {
        int c = 0;
        int len = mlen;
        int offset = 0;
     ...
     */
    private static int intArrayCmpToLen(final int[] arg1, final int[] arg2, final int len) {
        for (int i = 0; i < len; i++) {
     ...
     */
    private static int subN(final int[] a, final int[] b, final int ilen) {
        int len = ilen;
        long sum = 0;
     ...
     */
    static int mulAdd(final int[] out, final int[] in, final int offset, final int len, final int k) {
     ...
     * Parameters validation.
     */
    private static void implMulAddCheck(final int[] out, final int[] in, final int offset, final int len, final int k) {
        if (len > in.length) {
     ...
     * Java Runtime may use intrinsic for this method.
     */
    private static int implMulAdd(final int[] out, final int[] in, final int ioffset, final int len, final int k) {
        final long kLong = k & LONG_MASK;
        long carry = 0;

        int offset = out.length - ioffset - 1;
     ...
     */
    static int addOne(final int[] a, final int ioffset, int mlen, final int carry) {
        int offset = a.length - 1 - mlen - ioffset;
     ...
     */
    private BigInteger modPow2(final BigInteger exponent, final int p) {
        /*
     ...
     * and {@code p > 0}.
     */
    private BigInteger mod2(final int p) {
        if (bitLength() <= p) {
            return this;
        }
    ...
    int[] javaIncrement(final int[] ival) {
        int[] val = ival;
        int lastSum = 0;
    ...
    /**
     * Returns a BigInteger whose value is {@code (this & val)}. (This method returns a negative
     * BigInteger if and only if this and val are both negative.)
     *
    ...
    /**
     * Returns a BigInteger whose value is {@code (this | val)}. (This method returns a negative
     * BigInteger if and only if either this or val is negative.)
     *
    ...
    /**
     * Returns a BigInteger whose value is {@code (this ^ val)}. (This method returns a negative
     * BigInteger if and only if exactly one of this and val are negative.)
     *
    ...
    /**
     * Returns a BigInteger whose value is {@code (~this)}. (This method returns a negative value if
     * and only if this BigInteger is non-negative.)
     *
     * @return {@code ~this}
     */
    ...
    /**
     * Returns a BigInteger whose value is {@code (this & ~val)}. This method, which is equivalent
     * to {@code and(val.not())}, is provided as a convenience for masking operations. (This method
     * returns a negative BigInteger if and only if {@code this} is negative and {@code val} is
     * positive.)
     *
    ...
    /**
     * Returns a BigInteger whose value is equivalent to this BigInteger with the designated bit
     * cleared. (Computes {@code (this & ~(1<<n))}.)
     *
    ...
    /**
     * Returns a BigInteger whose value is equivalent to this BigInteger with the designated bit
     * flipped. (Computes {@code (this ^ (1<<n))}.)
     *
     ...
     *            - 1/2<sup>{@code certainty}</sup>). The execution time of this method is
     *            proportional to the value of this parameter.
     * @return {@code true} if this BigInteger is probably prime, {@code false} if it's definitely
     *         composite.
     */
     ...
     */
    final int compareMagnitude(final BigInteger val) {
        final int[] m1 = mag;
    ...
    /**
     * Version of compareMagnitude that compares magnitude with long value. val can't be
     * Long.MIN_VALUE.
     */
    final int compareMagnitude(final long ival) {
        assert ival != Long.MIN_VALUE;
        long val = ival;
        final int[] m1 = mag;
     ...
     */
    public String toString(final int iradix) {
        int radix = iradix;
        if (signum == 0) {
            return "0";
        }
     ...
     */
    private static void toString(final BigInteger u, final StringBuilder sb, final int radix,
                    final int digits) {
        /*
     ...
     */
    private static BigInteger getRadixConversionCache(final int radix, final int exponent) {
     ...
     * byte array will be in <i>big-endian</i> byte-order: the most significant byte is in the
     * zeroth element. The array will contain the minimum number of bytes required to represent this
     * BigInteger, including at least one sign bit, which is {@code (ceil((this.bitLength() +
     * 1)/8))}. (This representation is compatible with the {@link #BigInteger(byte[]) (byte[])}
     * constructor.)
     *
    ...
    public float floatValue() {
         ...
         * and signifFloor the top SIGNIFICAND_WIDTH.
         *
         * It helps to consider the real number signif = abs(this) * 2^(SIGNIFICAND_WIDTH - 1 -
         * exponent).
         */
         ...
         */
        final boolean increment = (twiceSignifFloor & 1) != 0 && ((signifFloor & 1) != 0 || abs().getLowestSetBit() < shift);
     ...
     */
    public static int[] trustedStripLeadingZeroInts(final int[] val) {
        final int vlen = val.length;
        int keep;
    ...
    /**
     * Takes an array a representing a negative 2's-complement number and returns the minimal (no
     * leading zero bytes) unsigned whose value is -a.
     */
    private static int[] makePositive(final byte[] a) {
        ...
        /*
         * Allocate output array. If all non-sign bytes are 0x00, we must allocate space for one
         * extra output byte.
         */
    ...
    /**
     * Takes an array a representing a negative 2's-complement number and returns the minimal (no
     * leading zero ints) unsigned whose value is -a.
     */
    private static int[] makePositive(final int[] a) {
        ...
        /*
         * Allocate output array. If all non-sign ints are 0x00, we must allocate space for one
         * extra output int.
         */
    ...
    /**
     * Returns the length of the two's complement representation in ints, including space for at
     * least one sign bit.
     */
     ...
     * @serialField bitLength int the number of bits in the minimal two's-complement representation
     *                  of this BigInteger
    ...
    /**
     * Reconstitute the {@code BigInteger} instance from a stream (that is, deserialize it). The
     * magnitude is read in as an array of bytes for historical reasons, but it is converted to an
     * array of ints and the byte array is discarded. Note: The current convention is to initialize
     * the cache fields, bitCount, bitLength and lowestSetBit, to 0 rather than some other marker
     * value. Therefore, no explicit action to set these fields needs to be taken in readObject
     * because those fields already have a 0 value be default since defaultReadObject is not being
     * used.
     */
    private void readObject(final java.io.ObjectInputStream s)
                    throws java.io.IOException, ClassNotFoundException {
        /*
         * In order to maintain compatibility with previous serialized forms, the magnitude of a
         * BigInteger is serialized as an array of bytes. The magnitude field is used as a temporary
         * store for the byte array that is deserialized. The cached computation fields should be
    ...
    private static class UnsafeHolder {
        ...
        static void putSign(final BigInteger bi, final int sign) {
            unsafe.putIntVolatile(bi, signumOffset, sign);
        }

        static void putMag(final BigInteger bi, final int[] magnitude) {
            unsafe.putObjectVolatile(bi, magOffset, magnitude);
        }
    }

    /**
     * Save the {@code BigInteger} instance to a stream. The magnitude of a BigInteger is serialized
     * as a byte array for historical reasons.
     *
     * @serialData two necessary fields are written as well as obsolete fields for compatibility
     *             with older versions.
     */
    ...
    /**
     * Converts this {@code BigInteger} to a {@code long}, checking for lost information. If the
     * value of this {@code BigInteger} is out of the range of the {@code long} type, then an
     * {@code ArithmeticException} is thrown.
     *
     * @return this {@code BigInteger} converted to a {@code long}.
     * @throws ArithmeticException if the value of {@code this} will not exactly fit in a
     *             {@code long}.
    ...
    /**
     * Converts this {@code BigInteger} to an {@code int}, checking for lost information. If the
     * value of this {@code BigInteger} is out of the range of the {@code int} type, then an
     * {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Converts this {@code BigInteger} to a {@code short}, checking for lost information. If the
     * value of this {@code BigInteger} is out of the range of the {@code short} type, then an
     * {@code ArithmeticException} is thrown.
     *
    ...
    /**
     * Converts this {@code BigInteger} to a {@code byte}, checking for lost information. If the
     * value of this {@code BigInteger} is out of the range of the {@code byte} type, then an
     * {@code ArithmeticException} is thrown.
     *
