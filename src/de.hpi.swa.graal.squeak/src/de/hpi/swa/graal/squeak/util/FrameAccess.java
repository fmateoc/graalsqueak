...
public final class FrameAccess {
    ...
    public static final class FrameArgumentsNode extends AbstractNodeWithCode {
        protected final int numRcvrAndArgs;
        @CompilationFinal private int stackPointer = -1;

        @Children private FrameSlotReadNode[] readNodes;

        private FrameArgumentsNode(final CompiledCodeObject code, final int argumentCount) {
            super(code);
            numRcvrAndArgs = 1 + argumentCount;
            readNodes = new FrameSlotReadNode[numRcvrAndArgs];
        }

        public static FrameArgumentsNode create(final CompiledCodeObject code, final int argumentCount) {
            return new FrameArgumentsNode(code, argumentCount);
        }

        @ExplodeLoop
        public Object[] execute(final VirtualFrame frame, final CompiledMethodObject method, final Object sender, final BlockClosureObject closure) {
            if (stackPointer == -1) {
                CompilerDirectives.transferToInterpreterAndInvalidate();
                stackPointer = FrameAccess.getStackPointer(frame, code) - numRcvrAndArgs;
                assert stackPointer >= 0 : "Bad stack pointer";
            }
            final Object[] frameArguments = new Object[ArgumentIndicies.RECEIVER.ordinal() + numRcvrAndArgs];
            assert method != null || sender != null : "Unexpected `null` value";
            frameArguments[ArgumentIndicies.METHOD.ordinal()] = method;
            frameArguments[ArgumentIndicies.SENDER_OR_SENDER_MARKER.ordinal()] = sender;
            frameArguments[ArgumentIndicies.CLOSURE_OR_NULL.ordinal()] = closure;
            for (int i = 0; i < numRcvrAndArgs; i++) {
                frameArguments[ArgumentIndicies.RECEIVER.ordinal() + i] = getReadNode(i).executeRead(frame);
                assert frameArguments[ArgumentIndicies.RECEIVER.ordinal() + i] != null;
            }
            FrameAccess.setStackPointer(frame, code, stackPointer);
            return frameArguments;
        }

        private FrameSlotReadNode getReadNode(final int offset) {
