...
import static de.hpi.swa.graal.squeak.math.BigDecimal.INFLATED;
...
class MutableBigInteger {
     ...
     */
    BigDecimal toBigDecimal(final int sign, final int scale) {
        if (intLen == 0 || sign == 0)
        ...
        final long v = len == 2 ? mag[1] & LONG_MASK | (d & LONG_MASK) << 32 : d & LONG_MASK;
     ...
     */
    private final int getInt(final int index) {
     ...
     * on all platforms.
     */
    private final long getLong(final int index) {
     ...
     * words.
     */
    private final void ensureCapacity(final int len) {
        if (value.length < len) {
            value = new int[len];
            offset = 0;
            intLen = len;
     ...
     * be adjusted.
     */
    private int divadd(final int[] a, final int[] result, final int offset) {
        long carry = 0;
     ...
     * dividend.
     */
    private int mulsub(final int[] q, final int[] a, final int x, final int len, int offset) {
     ...
     */
    private int mulsubBorrow(final int[] q, final int[] a, final int x, final int len, int offset) {
        final long xLong = x & LONG_MASK;
        long carry = 0;
        offset += len;
        for (int j = len - 1; j >= 0; j--) {
     ...
     * MutableBigInteger objects and the remainder object is returned.
     *
     * Uses Algorithm D in Knuth section 4.3.1. Many optimizations to that algorithm have been
     * adapted from the Colin Plumb C library. It special cases one word divisors for speed. The
     * content of b is not changed.
     *
     */
     ...
     * <a href="http://cr.yp.to/bib/1998/burnikel.ps"> Burnikel-Ziegler algorithm</a>. This method
    ...
    MutableBigInteger divideAndRemainderBurnikelZiegler(final MutableBigInteger b, final MutableBigInteger quotient) {
        ...
        if (r < s) {
            ...
            final int sigma = (int) Math.max(0, n32 - b.bitLength());   // step 3: sigma = max{T |
                                                                        // (2^T)*B < beta^n}
            ...
            final MutableBigInteger a1 = aShifted.getBlock(t - 1, t, n);   // the most significant
                                                                           // block of a

            // step 7: z[t-2] = [a[t-1], a[t-2]]
            MutableBigInteger z = aShifted.getBlock(t - 2, t, n);    // the second to most
                                                                     // significant block
     ...
     */
    private int divaddLong(final int dh, final int dl, final int[] result, final int offset) {
        long carry = 0;
     ...
     */
    private int mulsubLong(final int[] q, final int dh, final int dl, final int x, int offset) {
        final long xLong = x & LONG_MASK;
        offset += 2;
     ...
     */
    private boolean unsignedLongCompare(final long one, final long two) {
     ...
     */
    MutableBigInteger mutableModInverse(final MutableBigInteger p) {
     ...
     * changed by the calculation.
     *
     * This method implements an algorithm due to Richard Schroeppel, that uses the same
     * intermediate representation as Montgomery Reduction ("Montgomery Form"). The algorithm is
     * described in an unpublished manuscript entitled "Fast Modular Reciprocals."
     */
    private MutableBigInteger modInverse(final MutableBigInteger mod) {
